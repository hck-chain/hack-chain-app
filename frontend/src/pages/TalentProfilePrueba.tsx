...
const TalentProfile = () => {
  ...
  const { user } = useAuth();
  const { data: talent, isPending, isError } = useTalentProfile(wallet);
  ...
  const hasAnyLinks =
    !!talent?.website_url || !!talent?.linkedin_url || !!talent?.twitter_url;
  const profileUrl = `${window.location.origin}/talent/${wallet}`;
  return (
    <Layout>
      <GrainOverlay />
        ...
        >
          ...
          {talent && (
            ...
            >
                ...
                >
                  ...
                  <div className="relative px-6 sm:px-7 pb-6 sm:pb-7 -mt-10 sm:-mt-12">
                    <div className="flex flex-col sm:flex-row gap-5 sm:gap-6 sm:items-end">
                      ...
                      >
                        <AvatarImage
                          src={resolveIpfs(talent.photo_url)}
                          alt={displayName}
                        ...
                        >
                          {initials(talent.name, talent.lastname, talent.organization_name)}
                        </AvatarFallback>
                      </Avatar>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          {talent.is_approved && (
                            <BadgeCheck
                              ...
                              style={{ color: '#a855f7' }}
                              title={t('talentProfile.verifiedTooltip')}
                              aria-label={t('talentProfile.verifiedTooltip')}
                            />
                          )}
                        </div>
                        {talent.organization_name !== displayName && (
                          <p
                            className="text-sm font-medium mt-1"
                            style={{ color: P.textSecondary }}
                          >
                            {talent.organization_name}
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-5">
                      {talent.wallet_address && <WalletChip address={talent.wallet_address} />}
                      <div
                      ...
                      >
                        <img
                          ...
                          aria-hidden
                        />
                        {t('since')} {formatDate(talent.joined_at)}
                      </div>
                      <ShareButton 
                          url={profileUrl} 
                          ...
                          variant="outline"
                          onShare={() => api.registerProfileShare(talent.wallet_address)} 
                          />
                    </div>
                  </div>
                </article>
              ...
              {/* ── About ── */}
              <motion.div variants={itemVariants}>
                <Section label={t('talentProfile.aboutSection')}>
                  {talent.bio ? (
                    <p
              ...
              {/* ── Knowledge areas ── */}
              {talent.knowledge_areas.length > 0 && (
                <motion.div variants={itemVariants}>
                  <Section label={t('talentProfile.expertiseSection')}>
                    <div className="flex flex-wrap gap-2" role="list">
                      {talent.knowledge_areas.map((area) => (
              ...
              {hasAnyLinks && (
                <motion.div variants={itemVariants}>
                  <Section label={t('talentProfile.linksSection')}>
                    <div className="space-y-2.5">
                      {talent.website_url && (
                        <SocialLink
                          icon={<Globe className="h-4 w-4" />}
                          label={t('talentProfile.websiteLabel')}
                          url={talent.website_url}
                        />
                      )}
                      {talent.linkedin_url && (
                        <SocialLink
                          icon={<Linkedin className="h-4 w-4" />}
                          label={t('talentProfile.linkedinLabel')}
                      ...
                      {talent.twitter_url && (
                        <SocialLink
                          ...
                          label={t('talentProfile.twitterLabel')}
              ...
              <motion.div variants={itemVariants}>
                <Section
                  label={t('talentProfile.verificationSection')}
                    ...
                    >
                      <div className="min-w-0">
                        <div
                          ...
                          style={{ color: P.textMuted }}
                        >
                          {t('talentProfile.issuerWalletLabel')}
                        </div>
                      ...
                      <a
                        href={explorerUrl(talent.wallet_address)}
                        target="_blank"
                      ...
                      >
                        ...
                        <span className="hidden sm:inline">{t('talentProfile.viewOnExplorer')}</span>
                      </a>
                    </div>
